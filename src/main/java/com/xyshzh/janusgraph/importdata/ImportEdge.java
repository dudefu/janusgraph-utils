...
public class ImportEdge implements Task {
  ...
  public void execute(java.util.Map<String, Object> options) {
    ...
    ArrayList<String> fkeys = (ArrayList<String>) options.getOrDefault("fkeys", new String[] {}); // 如果不自定义id,则通过这些字段判断开始节点信息是否存在

    ArrayList<String> tkeys = (ArrayList<String>) options.getOrDefault("tkeys", new String[] {}); // 如果不自定义id,则通过这些字段判断结束节点信息是否存在
    ...
    filters.addAll(fkeys);
    filters.addAll(tkeys);
          ...
          try {
            while ((tempString = reader.readLine()) != "EXIT") {
              ...
              if (20 < tempString.length()) {
                ...
                if (content.containsKey("cardidFrom") && content.containsKey("cardidTo") && content.containsKey("label")) {
                  ...
                  // 判断关系是否存在
                  if (checkedge) {
                    HashMap<String, Object> kvs = new HashMap<String, Object>(); // 临时判定条件
...
//                        starts = starts.has(key.substring(0, key.length() - 1).replaceAll("label", "type"),
//                            content.get(key));
                        starts = starts.has("cardid", content.get(key));
                      }
                    }
...
//                        endVs.has(key.toString().substring(0, key.toString().length() - 1).replaceAll("label", "type"), content.get(key));
                        endVs.has("cardid", content.get(key));
                      }
                    }
                    if (starts.hasNext()) {
                      startV = starts.next();
